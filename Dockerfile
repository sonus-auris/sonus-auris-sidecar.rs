...
#
# Distroless image for sonus-auris-sidecar.
# Prefer linux/arm64:
...
#   - livenessProbe exec ["/usr/local/bin/sonus-auris-sidecar", "probe"]
#   - no readinessProbe
...
COPY --from=build --chown=65532:65532 "/usr/local/bin/sonus-auris-sidecar" "/usr/local/bin/sonus-auris-sidecar"
ENV SONUS_AURIS_SIDECAR_BIND=127.0.0.1:9090 \
    ORES_OTEL_SIDECAR_BIND=127.0.0.1:9090 \
    OTEL_SERVICE_NAME=sonus-auris-sidecar
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/sonus-auris-sidecar"]
